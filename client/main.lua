...
local function enterRobberyHouse(house)
    ...
    local data = exports['kg-interior']:CreateHouseRobbery(coords)
...
local function searchCabin(cabin)
    local ped = PlayerPedId()
    if math.random(1, 100) <= 85 and not KGCore.Functions.IsWearingGloves() then
    ...
    TriggerServerEvent('kg-houserobbery:server:SetBusyState', cabin, currentHouse, true)
    FreezeEntityPosition(ped, true)
    IsLockpicking = true
    local success = exports['kg-minigames']:Skillbar()
    if success then
        ClearPedTasks(PlayerPedId())
        TriggerServerEvent('kg-houserobbery:server:searchFurniture', cabin, currentHouse)
        Config.Houses[currentHouse]['furniture'][cabin]['searched'] = true
        TriggerServerEvent('kg-houserobbery:server:SetBusyState', cabin, currentHouse, false)
        FreezeEntityPosition(ped, false)
        IsLockpicking = false
    else
        ClearPedTasks(PlayerPedId())
        TriggerServerEvent('kg-houserobbery:server:SetBusyState', cabin, currentHouse, false)
        KGCore.Functions.Notify(Lang:t('error.process_cancelled'), 'error', 3500)
        FreezeEntityPosition(ped, false)
        IsLockpicking = false
    end
end
...
RegisterNetEvent('KGCore:Client:OnPlayerLoaded', function()
    KGCore.Functions.TriggerCallback('kg-houserobbery:server:GetHouseConfig', function(HouseConfig)
        Config.Houses = HouseConfig
    end)
end)

RegisterNetEvent('kg-houserobbery:client:ResetHouseState', function(house)
...
RegisterNetEvent('kg-houserobbery:client:enterHouse', function(house)
    enterRobberyHouse(house)
end)

RegisterNetEvent('kg-houserobbery:client:setHouseState', function(house, state)
    Config.Houses[house]['opened'] = state
end)

RegisterNetEvent('kg-houserobbery:client:setCabinState', function(house, cabin, state)
    Config.Houses[house]['furniture'][cabin]['searched'] = state
end)

RegisterNetEvent('kg-houserobbery:client:SetBusyState', function(cabin, house, bool)
...
RegisterNetEvent('lockpicks:UseLockpick', function(isAdvanced)
    if Config.UseClockHours then
        if GetClockHours() < Config.MinimumTime or GetClockHours() > Config.MaximumTime then
            KGCore.Functions.Notify(Lang:t('error.not_allowed_time'), 'error', 3500)
            return
        end
    end
        ...
        if CurrentCops >= Config.PoliceOnDutyRequired then
            if not Config.Houses[closestHouse]['opened'] then
                if not usingAdvanced then
                    if Config.RequireScrewdriver and not KGCore.Functions.HasItem('screwdriverset') then
                        KGCore.Functions.Notify(Lang:t('error.missing_something'), 'error', 3500)
                        return
                    end
                end
                ...
                if usingAdvanced then
                    local success = exports['kg-minigames']:Skillbar()
                    if success then
                        TriggerServerEvent('kg-houserobbery:server:enterHouse', closestHouse)
                        KGCore.Functions.Notify(Lang:t('success.worked'), 'success', 2500)
                    else
                        if math.random(1, 100) <= Config.ChanceToBreakAdvancedLockPick then
                            TriggerServerEvent('kg-houserobbery:server:removeAdvancedLockpick')
                            TriggerEvent('kg-inventory:client:ItemBox', KGCore.Shared.Items['advancedlockpick'], 'remove')
                        end
                        KGCore.Functions.Notify(Lang:t('error.didnt_work'), 'error', 2500)
                    end
                else
                    local success = exports['kg-minigames']:Skillbar('medium')
                    if success then
                        TriggerServerEvent('kg-houserobbery:server:enterHouse', closestHouse)
                        KGCore.Functions.Notify(Lang:t('success.worked'), 'success', 2500)
                    else
                        if math.random(1, 100) <= Config.ChanceToBreakLockPick then
                            TriggerServerEvent('kg-houserobbery:server:removeLockpick')
                            TriggerEvent('kg-inventory:client:ItemBox', KGCore.Shared.Items['lockpick'], 'remove')
                        end
                        KGCore.Functions.Notify(Lang:t('error.didnt_work'), 'error', 2500)
                    end
                end
            else
                KGCore.Functions.Notify(Lang:t('error.door_open'), 'error', 3500)
            end
        else
            KGCore.Functions.Notify(Lang:t('error.not_enough_police'), 'error', 3500)
        end
    end
end)
...
CreateThread(function()
    Wait(500)
    local requiredItems = {
        [1] = { name = KGCore.Shared.Items['advancedlockpick']['name'], image = KGCore.Shared.Items['advancedlockpick']['image'] },
        [2] = { name = KGCore.Shared.Items['screwdriverset']['name'], image = KGCore.Shared.Items['screwdriverset']['image'] },
    }
